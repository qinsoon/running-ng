...
def test_filter():
    assert filter_stats(lambda x: "foo" in x)(
        {"foo1": 1, "2foo": 2, "bar3": 3, "4bar": 4}
    ) == {"foo1": 1, "2foo": 2}


def test_reduce():
    assert (
        reduce_stats(r"\d*foo\d*", "foo_sum", lambda x, y: x + y)(
            {"foo1": 1, "2foo": 2, "bar3": 3, "4bar": 4}
        )["foo_sum"]
        == 3
    )
...
def test_sum_perf():
    assert (
        sum_work_perf_event("PERF_COUNT_HW_CPU_CYCLES")(
            {
        ...
        )["work.PERF_COUNT_HW_CPU_CYCLES.total"]
        == 3
    )

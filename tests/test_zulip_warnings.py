...
class TestZulipWarnings(unittest.TestCase):
    ...
    def test_not_moma_warning_removed(self):
        ...
        reservation = MomaReservaton(
            status=MomaReservationStatus.NOT_MOMA, user=None, start=None, end=None
        )

        # Mock the moma.get_reservation to return our test reservation
        with patch.object(
            self.zulip_plugin.moma, "get_reservation", return_value=reservation
        ):
    ...
    def test_other_reservation_warnings_preserved(self):
        ...
        # Test NOT_RESERVED
        reservation = MomaReservaton(
            status=MomaReservationStatus.NOT_RESERVED, user=None, start=None, end=None
        )
    ...
    def test_multiple_users_warning(self):
        ...
        with patch(
            "running.plugin.runbms.zulip.get_logged_in_users",
    ...
    def test_no_users_no_warning(self):
        ...
        with patch(
            "running.plugin.runbms.zulip.get_logged_in_users", return_value=set()
        ):
            message = self.zulip_plugin.get_user_warnings()
            self.assertEqual(message, "")
    ...
    def test_combined_warnings_in_message(self):
        ...
        with patch(
            ...
            with patch.object(
                ...
                with patch.object(
                    self.zulip_plugin.client, "send_message"
                ) as mock_send:
                    mock_send.return_value = {"result": "success", "id": 123}

                    # Set dry run to False for this test
                    with patch(
                        "running.plugin.runbms.zulip.is_dry_run", return_value=False
                    ):
